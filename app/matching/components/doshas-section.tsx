...
export default function DoshasSection({ doshas }: DoshasSectionProps) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="bg-gradient-to-br from-red-900/20 to-orange-900/20 border border-red-500/30 rounded-2xl p-5 md:p-8"
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-xl md:text-2xl font-semibold text-red-200">Doshas Detected</h3>
          <p className="text-xs md:text-sm text-red-300/70 mt-0.5">
      ...
      <div className="space-y-4 md:space-y-5">
        {parsedDoshas.map((dosha, i) => (
          <motion.div
            ...
            className="bg-gray-900/60 rounded-xl p-4 md:p-5 border border-gray-700/50"
          >
            <div className="flex items-start gap-3 mb-3 md:mb-4">
              <SeverityBadge severity={dosha.severity} />
              <h4 className="font-semibold text-white text-base md:text-lg flex-1">{dosha.name}</h4>
            </div>

            <div className="space-y-3 md:space-y-4 text-sm">
              <p className="text-gray-300 leading-relaxed text-xs md:text-sm">{dosha.impact}</p>

              <div className="grid md:grid-cols-2 gap-3 md:gap-4">
                <div className="bg-purple-900/20 rounded-lg p-3 md:p-4 border border-purple-500/20">
                  <div className="flex items-center gap-2 mb-2">
                    <Sparkles className="w-3.5 h-3.5 md:w-4 md:h-4 text-purple-400" />
                    <span className="font-medium text-purple-300 text-xs md:text-sm">Traditional Remedy</span>
                  </div>
                  <p className="text-gray-400 text-[11px] md:text-xs leading-relaxed">{dosha.remedy}</p>
                </div>

                <div className="bg-blue-900/20 rounded-lg p-3 md:p-4 border border-blue-500/20">
                  <div className="flex items-center gap-2 mb-2">
                    <Heart className="w-3.5 h-3.5 md:w-4 md:h-4 text-blue-400" />
                    <span className="font-medium text-blue-300 text-xs md:text-sm">Practical Approach</span>
                  </div>
                  <p className="text-gray-300 text-[11px] md:text-xs leading-relaxed">{dosha.practicalAdvice}</p>
                </div>
              </div>
            </div>
          </motion.div>
